...
public class BoardController {
    ...
    public String articlewrite(Model model, @RequestParam(defaultValue = "free") String boardcd) {
    ...
    public String articlewrite(Model model
        ...
        // 1. DB(tb_bbs_article) table insert. inserted pk 값을 반환 받는다.
        int insertedpk = srvboard.insertArticle(article);

        // 2. client의 파일을 server로 upload.
        if (!upload.getOriginalFilename().isEmpty()) {

            // 서버의 업로드 폴더 존재 여부 체크, 없으면 생성
            ...
            if (serverfile.exists()) {
                ...
                // 3. 성공한 경우만 tb_bbs_attachfile 테이블에 insert.
    ...
    public String articlemodify(Model model
        ...
        model.addAttribute("boardnm", srvboard.getBoardName(boardcd));
        model.addAttribute("boardcd", boardcd);
        model.addAttribute("boardarticlenocd", articleno);
        model.addAttribute("curPage", curPage);
        model.addAttribute("searchWord", searchWord);
    ...
    public String articlemodify(Model model, @ModelAttribute ModelArticle setValue,
            @RequestParam(value = "upload") MultipartFile upload, @RequestParam(defaultValue = "1") Integer curPage,
            @RequestParam(defaultValue = "") String searchWord) {
        ...
        if (!upload.getOriginalFilename().isEmpty()) {
            ...
            if (serverfile.exists()) {
                ...
                // 2. 성공한 경우만 tb_bbs_attachfile 테이블에 insert.
                ModelAttachFile attachFile = new ModelAttachFile();
        ...
        // 3. DB(tb_bbs_article) table update.
    ...
    @ResponseBody // REST서비스 이기 때문에 ajax를 이용
    public int deleteattachfile(Model model, @RequestParam Integer attachfileno) {
    ...
    public String articledelete(Model model
        ...
        if (result == 1) {
            url = String.format("redirect:/board/articlelist/%s?curPage=%dsearchWord=%s", boardcd, curPage, searchWord);
        }else{
