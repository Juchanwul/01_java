...
public class UserController {
    ...
    public String login(Model model, @RequestParam(value = "url", defaultValue = "") String url,
            HttpServletRequest request) {
        logger.info("/user/login");

        // 로그인 이전에 존재하던 페이지를 변수로 넘긴다.
        if (url.isEmpty())
    ...
    public String login(Model model, @RequestParam String url, @RequestParam String userid, @RequestParam String passwd,
        ...
        if (result != null) { // 로그인 성공
            session.setAttribute(/* 세션이름 */WebConstants.SESSION_NAME, /* 세션값 */result);
            if (url.isEmpty())
                return "redirect:/"; // -------------> http://loaclhost/ 가 열림
            else
                return "redirect:" + url;
    ...
    public String usermodify(Model model, HttpSession session) {
        ...
        // 로그인이 되어 있다는 과정. 세션에 사용자 정보가 들어 있음.
    ...
    public String usermodify(Model model, @ModelAttribute ModelUser setValue, HttpSession session, RedirectAttributes rttr) {
        logger.info("/user/usermodify : get");
