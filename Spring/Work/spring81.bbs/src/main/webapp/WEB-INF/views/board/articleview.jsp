...
<script>
	var goView = function(articleno) {
		location.href = '/board/articleview/${boardcd}/' + articleno + window.location.search;
	};
	...
	var goList = function( curPage, redirect ) {
		if( redirect == false ){
			location.href = '/board/articlelist/${boardcd}/${articleno}?curPage='+ curPage ;	
	...
	var goWrite = function() {
		location.href = '/board/articwrite/${boardcd}';
	};
				...
				<div id="bbs">
					<table>
						<tr>
							...
							<th style="width: 130px; color: #555;"><fmt:formatDate
									pattern="yyyy-MM-dd" value="${thisArticle.regdate }" /></th>
						</tr>
					</table>
						...
						<div class="fr">
							<c:if test="${nextArticle != null }">
								<input type="button" value="다음글"
									onclick="goView('${nextArticle.articleno }')" />
							</c:if>
							<c:if test="${prevArticle != null }">
								<input type="button" value="이전글"
									onclick="goView('${prevArticle.articleno }')" />
							</c:if>
						...
						<form id="searchForm" action="${actionurl}" method="get"
							...
							<p style="margin: 0; padding: 0;">
								<input type="hidden" name="boardcd" value="${boardcd }" /> <input
									type="text" name="searchWord" value="${searchWord}" size="15"
